...
elseif type(getexecutorname()) == 'string' and getexecutorname() == "ScriptWare" then
...
local hwid_list = {"Syn-Fingerprint", "Exploit-Guid", "Proto-User-Identifier", "Sentinel-Fingerprint", "Fingerprint", "Krnl-Hwid"};
local hwid = "";
...
do
	...
	for i = 1, #a do
		if (a[i]:IsA("Model") and a[i].Name == "Machine" and a[i]:FindFirstChild("ClickDetector")) then
...
local function JogOrRunToggle(a)
	if a == "Run" then
		game:GetService("Players").LocalPlayer.Backpack.Action:FireServer(remotesecurity, "RunToggle", {true, false})
	elseif a == "Jog" then
		game:GetService("Players").LocalPlayer.Backpack.Action:FireServer(remotesecurity, "RunToggle", {true, true})
	end
end
...
local function PPickStat(a, b)
	repeat wait() until game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild(a.."MachineGUI")
	local stam = game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild(a.."MachineGUI").Frame.ListF["Barbell "..b.." Weight"]
	game.VirtualInputManager:SendMouseButtonEvent(stam.AbsolutePosition.X + 100, stam.AbsolutePosition.Y + 50, 0,true,game,0)
	game.VirtualInputManager:SendMouseButtonEvent(stam.AbsolutePosition.X + 100, stam.AbsolutePosition.Y + 50, 0,false,game,0)
	game.VirtualInputManager:SendMouseButtonEvent(stam.AbsolutePosition.X + 100, stam.AbsolutePosition.Y + 50, 0,true,game,0)
	game.VirtualInputManager:SendMouseButtonEvent(stam.AbsolutePosition.X + 100, stam.AbsolutePosition.Y + 50, 0,false,game,0)
...
local function MuscleMachine(stat, level)
	wait(_G.TreadWait)
	local musclegen = _G.TreadRegen or 90
	wait(.75)
	print(stat, level)
	PPickStat(stat, level)
	wait(.75)
	repeat
		...
		wait()
	until percent >= musclegen
	Start()
	wait(.75)

	repeat wait()
		if not game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild(stat.."MachineGUI") then break end
		local liftf = game:GetService("Players").LocalPlayer.PlayerGui[stat.."MachineGUI"].Frame2.LiftingF
		if liftf:FindFirstChild("LiftIcon") then
			local lvl = liftf:FindFirstChild("LiftIcon")
			game.VirtualInputManager:SendMouseButtonEvent(lvl.AbsolutePosition.X + 30, lvl.AbsolutePosition.Y + 50, 0,true,game,0)
			game.VirtualInputManager:SendMouseButtonEvent(lvl.AbsolutePosition.X + 30, lvl.AbsolutePosition.Y + 50, 0,false,game,0)
		end
	until not game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild(stat.."MachineGUI")
end
	...
	for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
		if v:FindFirstChild("FoodScript") and v:IsA("Tool") then
			...
			if percentage < minimum then
				...
				if _G.RunToggle then
					JogOrRunToggle("Run")
				end

				if _G.JogToggle then
					JogOrRunToggle("Jog")
				end
				autoeat = false
			end
...
local function AutoSleep()
	...
	if char:FindFirstChild("Safe") then
		char:FindFirstChild("Safe"):Destroy();
	end
...
trainn:Toggle("Auto Striking Power [F1 to Disable]", false, function(a)
	_G.Test = a
	
	if _G.Test then
		game:GetService("UserInputService").InputBegan:Connect(function(input, gpe)
			if not _G.Test then return end
...
local drops2 = machine:Dropdown("Machine Type", {"Stamina", "RunningSpeed", "Squat", "Barbell"}, function(_type)
	_G.TreadmillType = _type
end)

local drops3 = machine:Dropdown("Machine Power", {"1", "2", "3", "4", "5", "6"}, function(power)
	_G.TreadmillPower = power
end)

local anotherslider = machine:Slider("Wait until % to start machine", 0, 100, 90, function(stam)
	_G.TreadRegen = stam
end)

machine:Toggle("Delay Machine", false, function(bool)
...
machine:Toggle("Auto Machine", false, function(bool)
	...
	while _G.TreadmillBool do
		...
		if _G.TreadmillType == "Stamina" or _G.TreadmillType == "RunningSpeed" then
			print('treedmals')
			repeat wait() until game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("TreadmillMachineGUI")
			Treadmill(_G.TreadmillType, _G.TreadmillPower)
		elseif _G.TreadmillType == "Barbell" then
			print('barball')
			repeat wait() until game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("BarbellMachineGUI")
			MuscleMachine(_G.TreadmillType, _G.TreadmillPower)
		elseif _G.TreadmillType == "Squat" then
			print("sqat")
			repeat wait() until game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("SquatMachineGUI")
			MuscleMachine(_G.TreadmillType, _G.TreadmillPower)
		end
		wait(.1);
	end
...
machine:Toggle("Auto Hopon Machine", false, function(bool)
	_G.AutoHopTread = bool

	while _G.AutoHopTread do
		if not _G.AutoHopTread then break end
		if not game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("TreadmillMachineGUI") or not  game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild(_G.TreadmillType.."MachineGUI")then
			for i,v in pairs(Treads) do
				local d = (root.Position - v:FindFirstChildWhichIsA("Part").Position).magnitude
...
misc:Toggle("Kick When Starving", false, function(fod)
	_G.KickWhenStarve = fod

	while _G.KickWhenStarve do
		if not _G.KickWhenStarve then break end
		local maxbar = game.Players.LocalPlayer.PlayerGui.MainGui.Utility.StomachBar.BarF
		local curbar = game.Players.LocalPlayer.PlayerGui.MainGui.Utility.StomachBar.BarF.Bar
		local hunger = (curbar.AbsoluteSize.X / maxbar.AbsoluteSize.X) * 100
...
misc:Button("RunToggle", function(c)
	_G.RunTog = true
	JogOrRunToggle("Run");
end)

misc:Button("JogToggle", function(c)
	_G.JogTog = true
	JogOrRunToggle("Jog");
end)
...
misc:Button("Transfer Flow Book", function()
	local vic = _G.Vic or nil
	local plr = game.Players.LocalPlayer
	local char = plr.Character
	local pos = char.HumanoidRootPart.Position
	...
	for i,v in pairs(plr.Backpack:GetChildren()) do
		if v:IsA("Tool") and v.Name == "Flow State Book" then
			v.Parent = char
		end
	end
...
misc:Textbox("Teleport To", "Username here", false, function(d)
	_G.TeleportTo = d
end)

misc:Button("Teleport to User", function()
	local plr = game.Players.LocalPlayer
	local vic = _G.TeleportTo or nil
	local deadpos
	local timestart
	...
	local function lawg()
		deadpos = game.Players[vic].Character:FindFirstChildWhichIsA("Part").CFrame * CFrame.new(0,5,0)
	end

	game.Players.LocalPlayer.Character.Humanoid.Died:Connect(lawg)
	game.Players.LocalPlayer.CharacterAdded:Connect(function(char)
		char:WaitForChild("Humanoid", 7).Died:Connect(lawg)
		repeat wait()
			if os.clock() - timestart > 8 then break end
			char:WaitForChild("HumanoidRootPart", 7).CFrame = game.Players[vic].Character:FindFirstChildWhichIsA("Part").CFrame * CFrame.new(0,5,0)
		until os.clock() - timestart > 8
	end)
end)
...
setting:Toggle("Mod Panic", false, function(panic)
	game.Players.PlayerAdded:Connect(function(player)
		if player:GetRoleInGroup(4800422) == "Mod" or player:GetRoleInGroup(4800422) == "MainChar" or player:GetRoleInGroup(4800422) == "sircroc!" or player:GetRoleInGroup(4800422) == "Associates." or player:GetRoleInGroup(4800422) == "Owner" or player.Name == "bIoodstainedforces" then
			...
			DiscordLib:Notification("KevHub", player.Name.. " joined KevHub is panicking", "Oh!")
		end 
	end)
end)
